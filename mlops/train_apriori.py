"""
Apriori Algorithm Training with W&B Experiment Tracking

This module trains the Apriori algorithm for association rule mining
...
def train_apriori(
    ...
    """
    Train Apriori algorithm with W&B tracking.
    ...
    output_dir = output_dir or APRIORI_DIR
    ...
    try:
        print("\n" + "="*60)
        print("APRIORI ALGORITHM TRAINING")
        print("="*60)
        ...
        # Run Apriori
        print(f"\n[3/5] Running Apriori (min_support={min_support})...")
        ...
        # Log rule table
        if len(rules) > 0:
            # Create summary table for W&B - convert frozensets to strings
        ...
        rules_file = os.path.join(output_dir, "apriori_rules.csv")
        ...
        artifact = wandb.Artifact(
            name="apriori-rules",
            type="model",
            description=f"Apriori rules with support>={min_support}, confidence>={min_confidence}, lift>={min_lift}",
            metadata={
                "algorithm": "Apriori",
                "num_rules": len(rules),
                "min_support": min_support,
        ...
        if len(frequent_itemsets) > 0:
            ...
            top_items_file = os.path.join(output_dir, "apriori_top_items.csv")
            top_items.to_csv(top_items_file, index=False)
